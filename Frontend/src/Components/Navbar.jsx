...
const Navbar = ({ activeIndexs }) => {
    ...
    const menuItem = [
        ...
        { name: "SHIRTS", link: "/products?type=shirts" },
        { name: "TROUSERS", link: "/products?type=trousers" },
        { name: "TOPS", link: "/products?type=tops" },
        { name: "JEANS", link: "/products?type=jeans" },
        { name: "SWEATSHIRTS", link: "/products?type=sweatshirts" },
        { name: "COATS", link: "/products?type=coat" },
        { name: "SHORTS | SKORTS", link: "/products?type=shorts" },
        { name: "CO-ORD SETS", link: "/products?type=co-ord_sets" },
        { name: "TSHIRTS", link: "/products?type=tshirt" },
    ];
    return (
        <>
            <Container theme={theme} style={{ backgroundColor: location.pathname === '/' ? 'transparent' : 'white' }}>
                <div className='menuContainer' style={{ backgroundColor: colorB }}>
                    <header className="header" style={{ backgroundColor: colorB }}>
                        ...
                        <Link to={`/products?gender=women`} state={{ query: 'women' }}>
                            <span>WOMEN</span>
                        </Link>
                        <Link to={`/products?gender=men`} state={{ query: 'men' }}>
                            <span>MEN</span>
                        </Link>
                        </div>
                        <ul className="menu" style={{overflow:"auto"}}>
                            {menuItem.map((item, index) => (
                                <li key={index} htmlFor="menu-btn">
                                    <Link to={item.link} style={item === 'Special Prices' ? { color: 'rgb(245, 57, 147)' } : {}}>{item.name}</Link>
                                </li>
                            ))}
                        </ul>
                    </header>
